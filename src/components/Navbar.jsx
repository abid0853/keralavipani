        ...
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center md:hidden">
            <button 
              ...
              className="p-2 text-gray-600 hover:bg-gray-50 rounded-lg transition-colors focus:outline-none"
            >
        ...
        <div className="md:hidden bg-white border-t border-gray-100 absolute w-full shadow-xl">
          <div className="px-4 pt-3 pb-6 space-y-2">
            <Link 
              to="/" 
              onClick={closeMenu}
              className={`block px-4 py-3 rounded-xl text-base font-bold transition-colors ${isActive('/') ? 'bg-emerald-50 text-primary' : 'text-gray-700 hover:bg-gray-50'}`}
            >
              Live Rates
            </Link>
            ...
            {currentUser ? (
              <>
                <Link 
                  to="/contribute" 
                  onClick={closeMenu}
                  className={`block px-4 py-3 rounded-xl text-base font-bold transition-colors ${isActive('/contribute') ? 'bg-emerald-50 text-primary' : 'text-gray-700 hover:bg-gray-50'}`}
                >
                  Add Price
                </Link>
                <Link 
                  to="/my-submissions" 
                  onClick={closeMenu}
                  className={`block px-4 py-3 rounded-xl text-base font-bold transition-colors ${isActive('/my-submissions') ? 'bg-emerald-50 text-primary' : 'text-gray-700 hover:bg-gray-50'}`}
                >
                  My Activity
                </Link>
                ...
                {userData?.role === 'admin' && (
                  <Link 
                    to="/admin" 
                    onClick={closeMenu}
                    className={`block px-4 py-3 rounded-xl text-base font-bold transition-colors ${isActive('/admin') ? 'bg-purple-50 text-purple-700' : 'text-gray-700 hover:bg-gray-50'}`}
                  >
                    Admin Panel
                  </Link>
                )}
                ...
                <div className="mt-4 pt-4 border-t border-gray-100">
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 text-red-600 bg-red-50 hover:bg-red-100 font-bold rounded-xl transition-colors"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
                    Sign Out
                  </button>
              ...
              <div className="pt-4">
                <Link 
                  to="/login" 
                  onClick={closeMenu}
                  className="block w-full text-center px-4 py-3 border border-transparent rounded-xl shadow-sm text-base font-bold text-white bg-primary hover:bg-emerald-600 transition-colors"
                >
                  Sign In
                </Link>
              </div>
